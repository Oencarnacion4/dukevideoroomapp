  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 pb-6">
      <div className="flex items-start justify-between">
        <div>
          <h4 className="font-(family-name:--font-heading) text-[21px] font-semibold">Resources</h4>
        ...
        {guides.map((g) => (
          <Link key={g.id} href={`/how-tos/${g.id}`}>
            <Card blueprint className="flex overflow-hidden p-0">
              {g.format === "document" ? (
                <div className="flex w-21 shrink-0 items-center justify-center border-r border-(--color-divider) bg-(--color-accent-100)">
                  <FileText size={22} strokeWidth={1.5} className="text-(--color-accent-700)" />
                </div>
              ) : (
                <StripedPlaceholder
              ...
              <div className="flex flex-1 flex-col justify-center gap-0.5 p-3">
                ...
                <span className="text-[11px] text-(--color-text-50)">
                  {g.format === "written" ? "Written" : g.format === "video" ? "Video" : "Document"} ·{" "}
                  {g.author?.full_name ?? "Unknown"}
                </span>
              </div>
            </Card>
          </Link>
